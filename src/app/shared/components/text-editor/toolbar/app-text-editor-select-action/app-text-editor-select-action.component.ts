...
@Component({
	selector: 'app-app-text-editor-select-action',
	standalone: true,
	imports: [
		MatFormFieldModule,
		MatSelectModule,
	...
	templateUrl: './app-text-editor-select-action.component.html',
	styleUrl: './app-text-editor-select-action.component.scss'
})
export class AppTextEditorSelectActionComponent {
	activeFormatsService = inject(ActiveFormatsService);
	formatter = inject(TextFormatterService);

	@Input({required: true}) format!: AllFormatOptions[2];
	...
	applyFormat(item: AllFormatOptions[2]["items"][number]) {
	...
	getSelectValue(item: AllFormatOptions[2]["items"][number]) {
